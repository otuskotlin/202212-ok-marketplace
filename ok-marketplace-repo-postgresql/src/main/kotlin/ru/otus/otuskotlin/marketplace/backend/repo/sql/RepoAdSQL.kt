...
class RepoAdSQL(
    ...
    override suspend fun createAd(rq: DbAdRequest): DbAdResponse {
        val ad = rq.ad.copy(
            lock = MkplAdLock(randomUuid()),
            id = rq.ad.id.takeIf { it != MkplAdId.NONE } ?: MkplAdId(randomUuid()))
        return save(ad)
    }
            ...
            val condition = Op.build {
                buildList {
                    ...
                    if (rq.ownerId != MkplUserId.NONE) {
                        add(AdsTable.ownerId eq rq.ownerId.asString())
                    }
                    if (rq.titleFilter.isNotBlank()) {
                        add((AdsTable.title like "%${rq.titleFilter}%") or
                            (AdsTable.description like "%${rq.titleFilter}%"))
                    }
                    if (rq.dealSide != MkplDealSide.NONE) {
                        add(AdsTable.adType eq rq.dealSide)
                    }
                }.reduce { a, b -> a and b }
            }

            val results = (AdsTable innerJoin UsersTable).select(condition)
